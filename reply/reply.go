...
func (r *MultiBulkReply) String() string {
	args := r.Args[1:]
	l := len(args)
	var argStrs = make([]string, l)
	for i := range args {
		argStrs[i] = string(args[i])
	}
	return fmt.Sprintf("{ cmd: %s, args: %v }", r.Args[0], argStrs)
}
...
type ScanBulkReply struct {
	Pos  int
	Args [][]byte
}

// MakeScanBulkReply creates MakeScanBulkReply
func MakeScanBulkReply(pos int, args [][]byte) *ScanBulkReply {
	return &ScanBulkReply{
		Pos:  pos,
		Args: args,
	}
}
...
// ToBytes marshal redis.Reply
func (r *ScanBulkReply) ToBytes() []byte {
	argLen := len(r.Args)
	var buf bytes.Buffer
	buf.WriteString("*2" + CRLF)
	if argLen == 0 {
		buf.WriteString("$1" + CRLF)
		buf.WriteString("0" + CRLF)
		buf.WriteString("*0" + CRLF)
	} else {
		l := len(strconv.Itoa(r.Pos))
		buf.WriteString("$" + strconv.Itoa(l) + CRLF)
		buf.WriteString(strconv.Itoa(r.Pos) + CRLF)
		buf.WriteString("*" + strconv.Itoa(argLen) + CRLF)
		for _, arg := range r.Args {
			if arg == nil {
				buf.WriteString("$-1" + CRLF)

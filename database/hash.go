...
func undoHSet(db *DB, args [][]byte) [][][]byte {
	key := string(args[0])
	field := string(args[1])
...
func undoHDel(db *DB, args [][]byte) [][][]byte {
	key := string(args[0])
	fields := make([]string, len(args)-1)
	fieldArgs := args[1:]
	for i, v := range fieldArgs {
...
func undoHMSet(db *DB, args [][]byte) [][][]byte {
	key := string(args[0])
	size := (len(args) - 1) / 2
	fields := make([]string, size)
	for i := 0; i < size; i++ {
...
func undoHIncr(db *DB, args [][]byte) [][][]byte {
	key := string(args[0])
	field := string(args[1])
	return rollbackHashFields(db, key, field)
}
...
func init() {
	RegisterCommand(cmd.HSet, execHSet, writeFirstKey, undoHSet, 4)
	RegisterCommand(cmd.HSetNX, execHSetNX, writeFirstKey, undoHSet, 4)
	RegisterCommand(cmd.HGet, execHGet, readFirstKey, nil, 3)
	RegisterCommand(cmd.HExists, execHExists, readFirstKey, nil, 3)
	RegisterCommand(cmd.HDel, execHDel, writeFirstKey, undoHDel, -3)
	RegisterCommand(cmd.HLen, execHLen, readFirstKey, nil, 2)
	RegisterCommand(cmd.HMSet, execHMSet, writeFirstKey, undoHMSet, -4)
	RegisterCommand(cmd.HMGet, execHMGet, readFirstKey, nil, -3)
	RegisterCommand(cmd.HGet, execHGet, readFirstKey, nil, -3)
	RegisterCommand(cmd.HKeys, execHKeys, readFirstKey, nil, 2)
	RegisterCommand(cmd.HVals, execHVals, readFirstKey, nil, 2)
	RegisterCommand(cmd.HGetAll, execHGetAll, readFirstKey, nil, 2)
	RegisterCommand(cmd.HIncrBy, execHIncrBy, writeFirstKey, undoHIncr, 4)

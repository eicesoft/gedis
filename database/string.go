...
func undoMSet(db *DB, args [][]byte) [][][]byte {
...
func execMSet(db *DB, args [][]byte) redis.Reply {
	...
	for i, key := range keys {
		value := values[i]
		db.PutEntity(key, &redis.DataEntity{Data: value})
	}
...
func execIncr(db *DB, args [][]byte) redis.Reply {
	...
	if bytes != nil {
		...
		db.PutEntity(key, &redis.DataEntity{
...
func init() {
	RegisterCommand(cmd.Set, execSet, writeFirstKey, rollbackFirstKey, -3)
	RegisterCommand(cmd.SetNx, execSetNX, writeFirstKey, rollbackFirstKey, 3)
	RegisterCommand(cmd.SetEX, execSetEX, writeFirstKey, rollbackFirstKey, 4)
	RegisterCommand(cmd.PSetEX, execPSetEX, writeFirstKey, rollbackFirstKey, 4)
	RegisterCommand(cmd.MSet, execMSet, prepareMSet, undoMSet, -3)
	RegisterCommand(cmd.MGet, execMGet, prepareMGet, nil, -2)
	RegisterCommand(cmd.MSetNX, execMSetNX, prepareMSet, undoMSet, -3)
	RegisterCommand(cmd.Get, execGet, readFirstKey, nil, 2)
	RegisterCommand(cmd.GetSet, execGetSet, writeFirstKey, rollbackFirstKey, 3)
	RegisterCommand(cmd.Incr, execIncr, writeFirstKey, rollbackFirstKey, 2)
	RegisterCommand(cmd.IncrBy, execIncrBy, writeFirstKey, rollbackFirstKey, 3)
	RegisterCommand(cmd.IncrByFloat, execIncrByFloat, writeFirstKey, rollbackFirstKey, 3)
	RegisterCommand(cmd.Decr, execDecr, writeFirstKey, rollbackFirstKey, 2)
	RegisterCommand(cmd.DecrBy, execDecrBy, writeFirstKey, rollbackFirstKey, 3)
	RegisterCommand(cmd.StrLen, execStrLen, readFirstKey, nil, 2)
	RegisterCommand(cmd.Append, execAppend, writeFirstKey, rollbackFirstKey, 3)
	RegisterCommand(cmd.SetRange, execSetRange, writeFirstKey, rollbackFirstKey, 4)
	RegisterCommand(cmd.GetRange, execGetRange, readFirstKey, nil, 4)
}

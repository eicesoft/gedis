...
func rollbackFirstKey(db *DB, args [][]byte) [][][]byte {
	key := string(args[0])
	return rollbackGivenKeys(db, key)
}

func rollbackGivenKeys(db *DB, keys ...string) [][][]byte {
...
func rollbackHashFields(db *DB, key string, fields ...string) [][][]byte {
	var undoCmdLines [][][]byte
...
func rollbackSetMembers(db *DB, key string, members ...string) [][][]byte {
	var undoCmdLines [][][]byte
...
func undoSetChange(db *DB, args [][]byte) [][][]byte {
	key := string(args[0])
	memberArgs := args[1:]
...
func rollbackZSetFields(db *DB, key string, fields ...string) [][][]byte {
	var undoCmdLines [][][]byte

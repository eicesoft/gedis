...
func execSInterStore(db *DB, args [][]byte) redis.Reply {
	...
	db.PutEntity(dest, &redis.DataEntity{
		Data: set,
	})
...
func init() {
	RegisterCommand(cmd.SAdd, execSAdd, writeFirstKey, undoSetChange, -3)
	RegisterCommand(cmd.SIsMember, execSIsMember, readFirstKey, nil, 3)
	RegisterCommand(cmd.SRem, execSRem, writeFirstKey, undoSetChange, -3)
	RegisterCommand(cmd.SCard, execSCard, readFirstKey, nil, 2)
	RegisterCommand(cmd.SMembers, execSMembers, readFirstKey, nil, 2)
	RegisterCommand(cmd.SInter, execSInter, prepareSetCalculate, nil, -2)
	RegisterCommand(cmd.SInterStore, execSInterStore, prepareSetCalculateStore, rollbackFirstKey, -3)
	RegisterCommand(cmd.SUnion, execSUnion, prepareSetCalculate, nil, -2)
	RegisterCommand(cmd.SUnionStore, execSUnionStore, prepareSetCalculateStore, rollbackFirstKey, -3)
	RegisterCommand(cmd.SDiff, execSDiff, prepareSetCalculate, nil, -2)
	RegisterCommand(cmd.SDiffStore, execSDiffStore, prepareSetCalculateStore, rollbackFirstKey, -3)
	RegisterCommand(cmd.SRandMember, execSRandMember, readFirstKey, nil, -2)
}

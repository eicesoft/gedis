...
func (dict *ConcurrentDict) getShardByKey(key string) *shard {
	if dict == nil {
		panic("dict is nil")
	}
	hashCode := fnv32(key)
...
func (dict *ConcurrentDict) Inc(key string, val int) (result int) {
	shard := dict.getShardByKey(key)
	shard.mutex.RLock()
	...
	if !exists {
		...
		d := v.(*int32)
		result = int(atomic.AddInt32(d, int32(val)))
	}
...
func (dict *ConcurrentDict) Exists(key string) (exists bool) {
	shard := dict.getShardByKey(key)
...
func (dict *ConcurrentDict) ForScanKeys(eachFn EachFunc, start int, count int) [][]byte {
	size := 0
	skip := 0
	var retKeys = make([][]byte, 0, count)
	shardCount := len(dict.table)
OuterLoop:
	for i := start; i < shardCount; i++ {
		shard := dict.getShard(uint32(i))
		if shard == nil {
			continue
		}

		for key, value := range shard.m {
			continues := eachFn(key, value)
			skip++
			if !continues {
				continue
			} else {
...
func (dict *ConcurrentDict) ForEach(eachFu EachFunc) {
	if dict == nil {
		panic("dict is nil")
	}

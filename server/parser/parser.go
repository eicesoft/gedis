...
// ParseStream 解析数据流
// 利用通道, 解析完一个请求包之后推到ch中. 包含一个Payload
...
func ParseOne(data []byte) (redis.Reply, error) {
	...
	reader := bytes.NewReader(data)
	go parse(reader, ch)
	payload := <-ch // parse will close the channel
	if payload == nil {
		return nil, errors.New("no reply")
	}
...
type readState struct {
	readingMultiLine  bool     //是否读取多行中
	expectedArgsCount int      //参数数量
	msgType           byte     //消息类型
	args              [][]byte //参数列表
	bulkLen           int64    //bulk长度
}
	...
	for {
		...
		if !state.readingMultiLine {
			...
			if msg[0] == '*' {
				...
				if err != nil {
					ch <- &Payload{
						Err: errors.New("parser error: " + string(msg)),
					}
...
func readLine(bufReader *bufio.Reader, state *readState) ([]byte, bool, error) {
	...
	if state.bulkLen == 0 { // read normal line
		...
		if len(msg) == 0 || msg[len(msg)-2] != '\r' {
			return nil, false, errors.New("parser error: " + string(msg))
		}
...
func parseMultiBulkHeader(msg []byte, state *readState) error {
	...
	if err != nil {
		return errors.New("parser error: " + string(msg))
	}
	if expectedLine == 0 {
...
func parseSingleLineReply(msg []byte) (redis.Reply, error) {
	...
	case ':': // int reply
		...
		if err != nil {
			return nil, errors.New("parser error: " + string(msg))
		}
		result = reply.MakeIntReply(val)
	default:
		// parse as text parser
